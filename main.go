...
func testProxy(proxy string, wg *sync.WaitGroup, results chan<- ProxyResult) {
	defer wg.Done()
	...
	if err == nil && resp.StatusCode == 200 {
		...
		results <- ProxyResult{proxy: proxy, responseTime: responseTime}
	}
}
...
func handleConnection(conn net.Conn, db *sql.DB) {
	...
	for i := 0; i < maxRetries; i++ {
		row := db.QueryRow("SELECT proxy FROM proxies WHERE response_time < 5000 ORDER BY RANDOM() LIMIT 1")
		var proxy string
		err := row.Scan(&proxy)
		if err != nil {
...
func main() {
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS proxies (id INTEGER PRIMARY KEY AUTOINCREMENT, proxy TEXT, response_time INTEGER, success_count INTEGER DEFAULT 0, last_test INTEGER)")
	if err != nil {
		log.Fatal(err)
	}
	...
	results := make(chan ProxyResult, 500)
	var wg sync.WaitGroup
	...
	for result := range results {
		testedCount++
		successCount++
		_, err = db.Exec("INSERT INTO proxies (proxy, response_time, success_count, last_test) VALUES (?, ?, 1, ?)",
			result.proxy, result.responseTime, time.Now().Unix())
	}
